...
def parse_args():
    ...
    parser.add_argument('--walks', type=str, required=True,
                        help='text file that has a random walk in each line. A random walk is just a seaquence of node ids separated by a space.')
    parser.add_argument('--types', type=str, required=True,
                        help='text file that has node types. each line is "node id <space> node type"')
    ...
    parser.add_argument('--embedding-dim', default=8, type=int, help='embedding dimensions')
...
def main(args):
    ...
    center_node_placeholder, context_node_placeholder, negative_samples_placeholder, loss = build_model(BATCH_SIZE=1,
                                                                                                        VOCAB_SIZE=len(
